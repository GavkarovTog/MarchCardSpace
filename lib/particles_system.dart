...
class PInterval<T> {
  ...
  PInterval.value(T value) : begin = value, end = value;
}
...
class ParticleManager {
  ...
  double _evaluateWithTimeShift(double t, double shift) {
    double efficientTime = t + shift;

    if (efficientTime > 1) {
      efficientTime -= 1;
      efficientTime = 1 - efficientTime;
    }
    ...
    for (int i = 0; i < particlesCount; i ++) {
      particles.add(
          Particle(
            x: rng.nextDouble(),
            y: rng.nextDouble(),
            dx: sin(rng.nextDouble() * 2 * pi) * _generateFromInterval(velocityRange),
            dy: cos(rng.nextDouble() * 2 * pi) * _generateFromInterval(velocityRange),
  ...
  void update() {
    ...
    for (int i = 0; i < particles.length; i ++) {
      ...
      if (fading) {
        currentParticle.opacity = opacityRange.begin + (opacityRange.end - opacityRange.begin) * _evaluateWithTimeShift(controller.drive(CurveTween(curve: Curves.easeOut)).value, currentParticle.timeShift);
      }
